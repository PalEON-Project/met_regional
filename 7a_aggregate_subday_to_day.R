...
outpath <- "/projectnb/dietzelab/paleon/met_regional/bias_corr/corr_timestamp_v2/daily"
...
for(v in 1:length(vars)){
  ...
  for(f in 1:length(files)){
	...
	dat.new <- array(dim=c(nrow(data.df), ncol(data.df), length(nc.time)))
    for(d in 1:length(nc.time)){
    	dat.new[,,d] <- apply(data.df[,,which(days == nc.time[d])], 1:2, mean, na.rm=F)
    }
    ...
    ofname  <- file.path(outpath,vars[v],paste0(vars[v],"_",sprintf('%04i',year),'_',
                   sprintf('%02i',mon),'.nc'))
