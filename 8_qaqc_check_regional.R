...
saveGIF( {  
  ...
  for(i in 1:length(tair.graph)){
	...
	}}, movie.name=file.path(dir.out, paste0("MetDrivers_MonthMeans", "_", yr.start1, "-", yr.end1, ".gif")), interval=0.3, nmax=10000, autobrowse=F, autoplay=F, ani.height=800, ani.width=800)
# ---------------------
...
# ---------------------
# Getting just the years for the time frame we're interested in
files.graph <-	which(as.numeric(substr(files.tair, 6,9))>=yr.start.mo1 & as.numeric(substr(files.tair, 6,9))<=yr.end.mo1)
...
# ---------------------
# Getting just the years for the time frame we're interested in
files.graph <-	which(as.numeric(substr(files.tair, 6,9))>=yr.start.mo2 & as.numeric(substr(files.tair, 6,9))<=yr.end.mo2)
...
saveGIF( {  
  ...
  for(i in files.graph){
	...
	plot.wind <- ggplot(data=wind.x) +
		...
		coord_equal(ratio=1)
	plot.time <- ggplot(data=tair.x) +
		geom_text(aes(x=1, y=1, label=paste(year, mon, day, hr, sep=" - ")), size=14) +
        theme(panel.background=element_blank()) 
...
# ---------------------
# Getting just the years for the time frame we're interested in
files.graph <-	which(as.numeric(substr(files.tair, 6,9))==yr.start.day2 & as.numeric(substr(files.tair, 11,12))==4)
...
saveGIF( {  
  ...
  for(i in files.graph){
	...
	plot.tair <- ggplot(data=tair.x) +
		geom_raster(aes(x=lon, y=lat, fill=tair)) +
		scale_fill_gradientn(colours=c("gray50", "red3"), limits=range(240,2900)) +
		geom_path(data=paleon.states, aes(x=long, y=lat, group=group)) +
		scale_x_continuous(limits=range(tair.x$lon), expand=c(0,0), name="Longitude") +
		scale_y_continuous(limits=range(tair.x$lat), expand=c(0,0), name="Latitude") +
		# ggtitle(paste("Tair", year, mon, sep=" - ")) +
	...
	plot.precipf <- ggplot(data=precipf.x) +
		geom_raster(aes(x=lon, y=lat, fill=precipf)) +
		scale_fill_gradientn(colours=c("gray50", "blue3"), limits=c(0,2e-3)) +
		geom_path(data=paleon.states, aes(x=long, y=lat, group=group)) +
		scale_x_continuous(limits=range(tair.x$lon), expand=c(0,0), name="Longitude") +
		scale_y_continuous(limits=range(tair.x$lat), expand=c(0,0), name="Latitude") +
		# ggtitle(paste("Precipf", year, mon, sep=" - ")) +
	...
	plot.swdown <- ggplot(data=swdown.x) +
		geom_raster(aes(x=lon, y=lat, fill=swdown)) +
		scale_fill_gradientn(colours=c("gray50", "goldenrod2"), limits=c(0,400)) +
		geom_path(data=paleon.states, aes(x=long, y=lat, group=group)) +
		scale_x_continuous(limits=range(tair.x$lon), expand=c(0,0), name="Longitude") +
		scale_y_continuous(limits=range(tair.x$lat), expand=c(0,0), name="Latitude") +
		# ggtitle(paste("Tair", year, mon, sep=" - ")) +
	...
	plot.lwdown <- ggplot(data=lwdown.x) +
		geom_raster(aes(x=lon, y=lat, fill=lwdown)) +
		# scale_fill_gradientn(colours=c("gray50", "darkorange1"), limits=c(0,330)) +
		scale_fill_gradientn(colours=c("gray50", "darkorange1"), limits=c(0,1450)) +
		geom_path(data=paleon.states, aes(x=long, y=lat, group=group)) +
		scale_x_continuous(limits=range(tair.x$lon), expand=c(0,0), name="Longitude") +
		scale_y_continuous(limits=range(tair.x$lat), expand=c(0,0), name="Latitude") +
		# ggtitle(paste("Tair", year, mon, sep=" - ")) +
	...
	plot.qair <- ggplot(data=qair.x) +
		geom_raster(aes(x=lon, y=lat, fill=qair)) +
		scale_fill_gradientn(colours=c("gray50", "aquamarine3"), limits=c(0, 0.01)) +
		geom_path(data=paleon.states, aes(x=long, y=lat, group=group)) +
		scale_x_continuous(limits=range(tair.x$lon), expand=c(0,0), name="Longitude") +
		scale_y_continuous(limits=range(tair.x$lat), expand=c(0,0), name="Latitude") +
		# ggtitle(paste("Tair", year, mon, sep=" - ")) +
	...
	plot.psurf <- ggplot(data=psurf.x) +
		geom_raster(aes(x=lon, y=lat, fill=psurf)) +
		scale_fill_gradientn(colours=c("gray50", "mediumpurple2"), limits=c(92000,102000)) +
		geom_path(data=paleon.states, aes(x=long, y=lat, group=group)) +
		scale_x_continuous(limits=range(tair.x$lon), expand=c(0,0), name="Longitude") +
		scale_y_continuous(limits=range(tair.x$lat), expand=c(0,0), name="Latitude") +
		# ggtitle(paste("Tair", year, mon, sep=" - ")) +
	...
	plot.wind <- ggplot(data=wind.x) +
		geom_raster(aes(x=lon, y=lat, fill=wind)) +
		scale_fill_gradientn(colours=c("gray80", "gray30"), limits=c(0,17)) +
		geom_path(data=paleon.states, aes(x=long, y=lat, group=group)) +
		scale_x_continuous(limits=range(tair.x$lon), expand=c(0,0), name="Longitude") +
		scale_y_continuous(limits=range(tair.x$lat), expand=c(0,0), name="Latitude") +
		# ggtitle(paste("Tair", year, mon, sep=" - ")) +
...
# ---------------------
# Getting just the years for the time frame we're interested in
files.graph <-	which(as.numeric(substr(files.tair, 6,9))==yr.start.day3 & as.numeric(substr(files.tair, 11,12))==4)
