...
for(v in 1:length(vars)){
  ...
  var.nchar <- nchar(vars[v])
  yrs       <- as.numeric(unique(substr(files, var.nchar+2, var.nchar+5)))
  
  for(y in yrs){ # Begin loop to make a single file for each year
    files2 <- files[which(substr(files, var.nchar+2, var.nchar+5)==y)]
  for(f in 1:length(files)){
    ...
    nc.file <- nc_open(paste(basedir,vars[v],"/",files2[f],sep=""))
    ...
    ofname  <- paste(outpath,vars[v],"/",vars[v],"_",sprintf('%04i',year),'.nc',sep="")
    ...
    rm(dat.new, nc.time, data.df, time.df) # clear out the key data objects so we start fresh each year
